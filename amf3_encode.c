...
static void encodeValue(smart_str *ss, zval *val, HashTable *sht, HashTable *oht TSRMLS_DC);
...
static void encodeStr(smart_str *ss, char *str, int len, HashTable *ht TSRMLS_DC) {
...
static void encodeArray(smart_str *ss, zval *val, HashTable *sht, HashTable *oht TSRMLS_DC) {
	...
	if (len == zend_hash_num_elements(ht)) { /* dense array */
		...
		for (zend_hash_internal_pointer_reset_ex(ht, &hp) ;; zend_hash_move_forward_ex(ht, &hp)) {
			...
			if (!len--) break;
			encodeValue(ss, *hv, sht, oht TSRMLS_CC);
		}
...
static void encodeObject(smart_str *ss, zval *val, HashTable *sht, HashTable *oht TSRMLS_DC) {
...
static void encodeValue(smart_str *ss, zval *val, HashTable *sht, HashTable *oht TSRMLS_DC) {
	switch (Z_TYPE_P(val)) {
		...
		case IS_ARRAY: {
			...
			encodeArray(ss, val, sht, oht TSRMLS_CC);
			break;
		}
		case IS_OBJECT: {
			smart_str_appendc(ss, AMF3_OBJECT);
			encodeObject(ss, val, sht, oht TSRMLS_CC);
			break;
		}
	}
}
...
PHP_FUNCTION(amf3_encode) {
	...
	encodeValue(&ss, val, &sht, &oht TSRMLS_CC);
